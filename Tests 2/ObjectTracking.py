...
#model_dir = os.path.join(os.path.expanduser("~"), "Desktop", "all_models")
...
#---------Flask----------------------------------------
#THIS IS FOR VIDEO FEED ON WEB BROWSER - MIGHT NOT NEED
...
def track_object(objs,labels): 
    #objs: list of detected objects, should contain Bbox, confidence score etc
    #labels: dictionary where keys are object IDs and values are corr. labels

    #global delay
    global x_deviation, y_deviation, tolerance, arr_track_data
    
    
    if(len(objs)==0):# If no objects are detected, sotp the robot
        print("no objects to track")
        ut.stop()
        #ut.red_light("OFF")
        arr_track_data=[0,0,0,0,0,0] #reset the tracking data
        return
    ...
    k=0
    flag=0
    for obj in objs: #loop through detected objects
        lbl=labels.get(obj.id, obj.id)
        k = arr_valid_objects.count(lbl) #check if detected object is in valud object list
        if (k>0): #if we have detected 1 or more objects in our object list
            x_min, y_min, x_max, y_max = list(obj.bbox) #obtain bounding box
            flag=1 # set flag to 1 if object found
            break
    ...
    x_diff=x_max-x_min #calculate the width of bounding box
    y_diff=y_max-y_min #calculate height of bounding box
    print("x_diff: ",round(x_diff,5)) 
    print("y_diff: ",round(y_diff,5))
        
        
    obj_x_center=x_min+(x_diff/2) #calculate x-coordinate of object center
    obj_x_center=round(obj_x_center,3) 
    
    obj_y_center=y_min+(y_diff/2) #calculate y-coordinate of object center
    obj_y_center=round(obj_y_center,3)
    
        
    print("[",obj_x_center, obj_y_center,"]")
        
    x_deviation=round(0.5-obj_x_center,3) #calculate deviation deviation of the object from center of the frame
    y_deviation=round(0.5-obj_y_center,3) #normalised picture --> frame = 0.5 
        
    print("{",x_deviation,y_deviation,"}")
   
    #move_robot()
    thread = Thread(target = move_robot) #run move_robot function concurrently 
    thread.start() #begin thread execution
    #thread.join()

    #print(cmd)
    #filling up arr_track_data with object center coordinates and deviation from center in both coordinates
...
def move_robot():
    ...
    #check if robot is within tolerance range for both x and y deviations
    if(abs(x_deviation)<tolerance and abs(y_deviation)<tolerance):
        cmd="Stop" #command is stop if the robot is centered within tolerence
        delay1=0 #no delay since robot doesnt need to move
        ut.stop() #stop robot
        #ut.red_light("ON")
        ...
        if (abs(x_deviation)>abs(y_deviation)):
            #adjust robot based on x deviation
            if(x_deviation>=tolerance): #if the x_deviation >tolerance, move robot left to align
                cmd="Move Left" #set command to move left
                delay1=get_delay(x_deviation,'l') #calculate delay based on x deviation
                
                ut.left() #move robot left
                time.sleep(delay1)#move robot for calculated duration
                ut.stop() #stop robot
                
            if(x_deviation<=-1*tolerance):
                cmd="Move Right" #set command to move right
            ...
            #Now adjust robot position based on y deviation
    ...
    #filling arr_track_data with command to execute and delay time
...
def get_delay(deviation,direction):
    ...
    if (direction=='f' or direction=='b'):
        if(deviation>=0.3): # high deviation leads to higher delays 
            d=0.1
...
def main():
    #import the edgetpu flag from the util file
    ...
    #load the model interpreter and labels using chosen model and edgetpu
